...
// Obtener fecha actual en formato YYYY-MM-DD en la zona horaria de Chile (reinicio diario a las 00:00)
...
function loadVisits() {
  try {
    ...
    if (!Array.isArray(data.daily.uniqueIPs)) {
      data.daily.uniqueIPs = [];
    }
    if (typeof data.daily.todayVisitors !== 'number') {
      data.daily.todayVisitors = data.daily.uniqueIPs.length;
    }
    ...
    const today = getChileDateString();
    return {
      totalPageViews: lastKnownTotalPageViews,
      uniqueIPs: [],
      daily: {
        date: today,
        todayVisitors: 0,
...
// 1. Cabeceras de seguridad HTTP con Helmet (Configuración compatible con Google Fonts, Google Ads, GTM y HSTS)
app.use(helmet({
  contentSecurityPolicy: {
    directives: {
      ...
      scriptSrc: ["'self'", "'unsafe-inline'", "https://www.googletagmanager.com", "https://www.google-analytics.com", "https://googleads.g.doubleclick.net"],
      connectSrc: [
        "'self'",
        "http://localhost:3000",
        "ws://localhost:5173",
...
app.post('/api/visits', visitsLimiter, (req, res) => {
  ...
  // 2. Cargar y actualizar visitas totales y diarias
  const visitsData = loadVisits();
  const today = getChileDateString();

  // Reiniciar a 0 si cambiaron las 00:00
  if (!visitsData.daily || visitsData.daily.date !== today) {
    visitsData.daily = {
      date: today,
      todayVisitors: 0,
      uniqueIPs: []
  ...
  // Contar visitantes únicos de hoy
  if (!visitsData.daily.uniqueIPs.includes(ipHash)) {
    visitsData.daily.uniqueIPs.push(ipHash);
    visitsData.daily.todayVisitors = visitsData.daily.uniqueIPs.length;
  }
  ...
  res.json({
    ...
    todayVisitors: visitsData.daily.todayVisitors || visitsData.daily.uniqueIPs.length
  });
});
...
app.get('/api/visits', visitsLimiter, (req, res) => {
  ...
  // Comprobar si pasaron las 00:00 para reiniciar a 0
  if (!visitsData.daily || visitsData.daily.date !== today) {
    visitsData.daily = {
      date: today,
      todayVisitors: 0,
