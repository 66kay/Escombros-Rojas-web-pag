...
// ==========================================
// SEGUIMIENTO DEFENSIVO DE CONVERSIONES (GOOGLE ADS / GA4)
// ==========================================
function trackConversion(eventName, eventParams = {}) {
  try {
    ...
    // Falla silenciosa: nunca interrumpe la navegación ni el envío
    console.debug('[Tracking]', err);
  }
}
...
// Seguimiento defensivo de clics a WhatsApp (Conversión clave en Google Ads)
document.querySelectorAll('a[href*="wa.me"]').forEach(link => {
  link.addEventListener('click', () => {
    trackConversion('contact_whatsapp', {
      event_category: 'WhatsApp',
      event_label: 'Clic Contacto WhatsApp'
    });
  });
});

// Seguimiento defensivo de llamadas directas (Conversión clave en Google Ads)
document.querySelectorAll('a[href^="tel:"]').forEach(link => {
  link.addEventListener('click', () => {
    trackConversion('contact_phone', {
      event_category: 'Llamada',
      event_label: 'Clic Llamada Telefonica'
    });
  });
});
...
async function trackVisit(isInitial = false) {
  try {
    ...
    if (response.ok) {
      const data = await response.json();
      updateVisitorUI(data.activeOnline, data.totalPageViews, data.todayVisitors);
    }
  } catch (error) {
...
function updateVisitorUI(active, total, todayVisitors) {
  ...
  const todayCounterFooter = document.getElementById('today-counter-footer');

  if (onlineCounter) onlineCounter.textContent = active;
  if (totalCounter) totalCounter.textContent = total;
  if (todayCounter) todayCounter.textContent = todayVisitors !== undefined ? todayVisitors : '--';
  if (onlineCounterFooter) onlineCounterFooter.textContent = active;
  if (totalCounterFooter) totalCounterFooter.textContent = total;
  if (todayCounterFooter) todayCounterFooter.textContent = todayVisitors !== undefined ? todayVisitors : '--';
}
